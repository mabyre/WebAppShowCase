...
namespace UserControls
{
    ...
    public class CommentViewBase : UserControl
    {
        ...
        protected string AdminLinks
        {
            get
            {
                if ( Page.User.IsInRole( "Administrateur" ) || Page.User.Identity.Name.Equals( Post.Author ) )
                {
                    ...
                    string confirmDelete = string.Format( CultureInfo.InvariantCulture, "Vous êtes sûr ?", "delete" , "the comment" );
                    sb.AppendFormat( " | <a href=\"javascript:void(0);\" onclick=\"if (confirm('{1}')) location.href='?deletecomment={0}'\">{2}</a>", Comment.Id, confirmDelete, "delete" );

                    if ( Comment.IsApproved == false )
                    {
                        sb.AppendFormat( " | <a href=\"?approvecomment={0}\" class=\"LabelApprouveRedStyle\">{1}</a>", Comment.Id, "approve" );
                ...
                {
                    System.Text.StringBuilder sb = new System.Text.StringBuilder();
                    if ( this.Comment.IsApproved == false )
                        sb.Append( "<label class=\"LabelApprouveRedStyle\">non approuvé</label>" );
                    return sb.ToString();
                }
                return string.Empty;
            }
        ...
        protected string Flag
        {
            get
            {
                ...
                {
                    string path = Server.MapPath( "~/Images/flags/" + Comment.Country + ".png" );
                    if ( File.Exists( path ) )
                    {
                        return "<img src=\"" + Utils.RelativeWebRoot + "Images/flags/" + Comment.Country + ".png\" class=\"flag\" alt=\"" + Comment.Country + "\" />";
                    }
                }
        ...
        {
            if ( Global.SettingsXml.Avatar == "none" )
                return null;

            if ( String.IsNullOrEmpty( Comment.Email ) || Comment.Email.Contains( "@" ) == false )
            {
                ...
                return "<img src=\"" + Utils.AbsoluteWebRoot + "App_Themes/Sodevlog/Images" + /*BlogSettings.Instance.Theme +*/ "/noavatar.jpg\" alt=\"" + Comment.Author + "\" />";
            }

            string img = "<img src=\"{0}\" alt=\"{1}\" />";
            string hash = CreateMd5Hash();
            string noavatar = Utils.AbsoluteWebRoot + "App_Themes/Sodevlog/Images/noavatar.jpg";
            ...
            switch ( Global.SettingsXml.Avatar )
            {
                case "monster":
